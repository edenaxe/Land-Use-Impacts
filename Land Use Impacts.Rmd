...
# Returns a raster
raster_file <- map(.x = vector_file$Name, 
                   .f = function(x) {
                     
                     proj <- vector_file %>% sf::st_as_sf() %>% filter(Name == x)
...
# WORKS!!! Just need to figure out the plots now
test_tbl <- raster_file %>%
  mutate(Area_Acres = coverage_fraction*900*0.000247105) %>%
  group_by(value, Name) %>%
...
dest_path <- paste0(getwd(), "/Data/NLCD 2019 Land Cover.zip")
dl_url <- "https://s3-us-west-2.amazonaws.com/mrlc/nlcd_2019_land_cover_l48_20210604.zip"
download.file(dl_url, destfile = dest_path)
unzip(dest_path, exdir = paste0(getwd(), "/Data/NLCD 2019 Land Cover"))
